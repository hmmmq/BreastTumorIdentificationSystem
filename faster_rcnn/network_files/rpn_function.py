...
class AnchorsGenerator(nn.Module):
    ...
    def __init__(self, sizes=(128, 256, 512), aspect_ratios=(0.5, 1.0, 2.0)): #anchor面积大小和比例
        super(AnchorsGenerator, self).__init__()

        if not isinstance(sizes[0], (list, tuple)): # 类型判断
            # TODO change this
        ...
        self._cache = {} # 空字典，存放所有anchor的坐标信息。
    ...
    def generate_anchors(self, scales, aspect_ratios, dtype=torch.float32, device=torch.device("cpu")):
        ...
        h_ratios = torch.sqrt(aspect_ratios) #高度对应的乘法因子
        w_ratios = 1.0 / h_ratios #宽度对应的乘法因子

        # [r1, r2, r3]' * [s1, s2, s3]
        # number of elements is len(ratios)*len(scales)
        ws = (w_ratios[:, None] * scales[None, :]).view(-1) #求anchor的高
        hs = (h_ratios[:, None] * scales[None, :]).view(-1) #求anchor的宽
...
class RPNHead(nn.Module): # 得到分类矩阵和回归矩阵
    """
    ...
    def __init__(self, in_channels, num_anchors):
        ...
        # 计算预测的目标分数（这里的目标只是指前景或者背景）1*1的卷积，num_anchors=9
        self.cls_logits = nn.Conv2d(in_channels, num_anchors, kernel_size=1, stride=1)
        # 计算预测的目标bbox regression参数
        self.bbox_pred = nn.Conv2d(in_channels, num_anchors * 4, kernel_size=1, stride=1)

        for layer in self.children(): #参数的初始化
            if isinstance(layer, nn.Conv2d):
                torch.nn.init.normal_(layer.weight, std=0.01)
                torch.nn.init.constant_(layer.bias, 0)

    def forward(self, x): # x是backbone生成的多个预测特征层
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def __init__(self, anchor_generator, head, #对需要用到的变量的注释
                 fg_iou_thresh, bg_iou_thresh, #rpn计算损失时，采集正负样本设置的阈值。default：0.7 0.3
                 batch_size_per_image, positive_fraction, # rpn再计算损失时采用正负样本的总个数，正样本的比例 256，0.5
                 pre_nms_top_n, post_nms_top_n, # nms前保留的proposals个数，nms后。2000。2000
                 nms_thresh, score_thresh=0.0): # nms处理阈值。0.7
        ...
        self.box_similarity = box_ops.box_iou #计算iou值，两个proposal的重叠程度。

        self.proposal_matcher = det_utils.Matcher(  # 划分正负样本。
            fg_iou_thresh,  # 当iou大于fg_iou_thresh(0.7)时视为正样本
            bg_iou_thresh,  # 当iou小于bg_iou_thresh(0.3)时视为负样本
            allow_low_quality_matches=True
        )

        self.fg_bg_sampler = det_utils.BalancedPositiveNegativeSampler( # 采集计算rpn网络损失的正负样本。
        ...
        self.min_size = 1. #过滤proposals时使用的参数
