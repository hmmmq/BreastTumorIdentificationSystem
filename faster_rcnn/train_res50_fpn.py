...
def main(parser_data):
    device = torch.device(parser_data.device if torch.cuda.is_available() else "cpu") #指定训练设备
    ...
    # load validation data set 读取验证集
    ...
    model.to(device) #把模型导入到GPU

    # define optimizer。 SGD 只使用一个batch进行梯度下降
    params = [p for p in model.parameters() if p.requires_grad]
    optimizer = torch.optim.SGD(params, lr=0.005,
                                momentum=0.9, weight_decay=0.0005)

    # 学习率的退货策略 learning rate scheduler 每隔step_size步将学习率乘以gamma
...
if __name__ == "__main__":
    ...
    # 训练设备类型
    parser.add_argument('--device', default='cuda:0', help='device') #cuda=gpu
    # 训练数据集的根目录(VOCdevkit)
    ...
    # 训练的总epoch数 所有图像处理一次才叫一个epoch，可以调参
    parser.add_argument('--epochs', default=20, type=int, metavar='N',
                        help='number of total epochs to run')
    # 训练的batch size ,一次迭代只能处理的图像的数量。根据电脑配置调参
